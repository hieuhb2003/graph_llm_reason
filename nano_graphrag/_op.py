...
async def extract_entities(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        # 1. Put chunk_content into LLM to extract entities and relationships
        hint_prompt = entity_extract_prompt.format(**context_base, input_text=content)
        final_result = await use_llm_func(hint_prompt)
        # ---

        # 2. Packing with previous history then other refining based on the max_gleaning
        ...
        # 3. Postprocess the text of extracted entities and relationsips into 
        #   nodes : dict : {entity_name : [<attribute of entity>, <attribute of entity>, ...]}
        #   edges : dict : {(source_entity, target_entity) : [<attribute of relationships> , <attribute of relationships>]}
    ...
    #     results.append(await _process_single_content(c))

    print()  # clear the progress bar
    maybe_nodes = defaultdict(list)
    maybe_edges = defaultdict(list)
    
    # 4. Group information of same entities and same relationships
    ...
    # 5. Merge information and push upsert it into knowledge graph
    ...
    #6. if using local search -> using entity_vdb for storing vector of entity name + entity description.
    if entity_vdb is not None:
        logger.info("Storing the entity embedding into entity vector db")
        data_for_vdb = {
        ...
        logger.info("Storing the edge embedding into edges vector db")
        data_for_vdb = {
            compute_mdhash_id(dp["src_id"] + dp["tgt_id"], prefix="edg-"): {
        ...
        await edge_vdb.upsert(data_for_vdb)
...
async def generate_community_report(
    ...
    for level in levels:
        ...
        for c in this_level_community_values:
            this_level_communities_reports.append(await _form_single_community_report(c, community_datas))
...
async def _find_most_related_text_unit_from_entities(
    ...
    # edges_data = await asyncio.gather(
    #     *[knowledge_graph_inst.get_node_edges_data(dp["entity_name"]) for dp in node_datas]
    # )
    ...
    # print([node["entity_name"] for node in node_datas])
    # print(text_units)
    query_embedding = await global_config["embedding_func"]([query])                                        
        ...
        for c_id in this_text_units:
            ...
            for e in this_edges:
                if (
                    ...
                    # edge_description = knowledge_graph_inst.get_edges_data(e[0], e[1])["description"]
                    weight = knowledge_graph_inst.get_edges_data(e[0], e[1])["weight"]
                    # description_embedding = await global_config["embedding_func"]([edge_description])
                    # query_embedding = await global_config["embedding_func"]([query])                                        
                    # relation_counts += 1 * float(np.dot(description_embedding.reshape(-1), query_embedding.reshape(-1)))
                    embedding_edge_idx1 = compute_mdhash_id(e[0] + e[1], prefix="edg-")
                    embedding_edge_idx2 = compute_mdhash_id(e[1] + e[0], prefix="edg-")
                    edge_embedding = await edges_vdb.get([embedding_edge_idx1, embedding_edge_idx2])

                    relation_counts += float(np.dot(edge_embedding.reshape(-1), query_embedding.reshape(-1)))
                    # relation_counts += 1
...
async def extract_keywords_and_query(
  query,
  ...
  global_config : dict
    ...
    # Extract keywords:
    llm = global_config["best_model_func"]
    prompt_variable = {
        "query" : query,
        "history" : [],
        "examples" : PROMPTS["keyword_examples"]
    }
    llm_keywords_string = await llm(PROMPTS["query_keywords_extraction"].format(**prompt_variable))
    llm_keywords_dict = ast.literal_eval(llm_keywords_string)
    local_keywords, global_keywords = llm_keywords_dict["low_level_keywords"],llm_keywords_dict["high_level_keywords"]

    # From local_keywords, extract nodes for each keywords
    node_datas_keyword = await asyncio.gather(
        *[entities_vdb.query(keyword, top_k=query_param.top_k) for keyword in local_keywords]
    )

    name_entity_distance = {}
    for nodes_each_keyword in node_datas_keyword:
        for node in nodes_each_keyword:
            entity_name = node["entity_name"]
            distance = node["distance"]
            if entity_name in name_entity_distance:
                name_entity_distance[entity_name]["distance"] += distance
            else:
                name_entity_distance[entity_name] = node

    name_entity_distance_tuple_sorted = sorted(list(name_entity_distance.values()), key = lambda x :x["distance"], reverse=True)

    name_entity_distance_top_k = name_entity_distance_tuple_sorted[:query_param.top_k]


    return name_entity_distance_top_k
...
async def _build_local_query_context(
    ...
    # keyword_results = await extract_keywords_and_query(query, 
    #                                                    entities_vdb,
    #                                                    query_param,
    #                                                    global_config)
    results = await entities_vdb.query(query, top_k=query_param.top_k)
    # print(query)
    # print([r["entity_name"] for r in results])
    if not len(results):
        return None
    node_datas = await asyncio.gather(
        *[knowledge_graph_inst.get_node(r["entity_name"]) for r in results]
    )
    # node_datas_keywords = await asyncio.gather(
    #     *[knowledge_graph_inst.get_node(r["entity_name"]) for r in keyword_results]
    # )
    ...
    # node_degrees_keywords = await asyncio.gather(
    #     *[knowledge_graph_inst.node_degree(r["entity_name"]) for r in keyword_results]
    # )
    ...
    # node_datas_keywords = [
    #     {**n, "entity_name": k["entity_name"], "rank": d}
    #     for k, n, d in zip(keyword_results, node_datas_keywords, node_degrees_keywords)
    #     if n is not None
    # ]
    ...
    # )
    use_communities = []
    use_text_units = await _find_most_related_text_unit_from_entities(query,
        node_datas, query_param, text_chunks_db, knowledge_graph_inst, global_config, edges_vdb
    )
    ...
    list_of_list_store_json(query, text_units_section_list,global_config["store_query_text_chunks"])
    return f"""
-----Reports-----
```csv
{communities_context}
...
async def local_query(
    ...
    directory_path = os.path.join(global_config["working_dir"], "local_query")
...
async def naive_query(
    ...
    directory_path = os.path.join(global_config["working_dir"], "naive_query")
    if not os.path.exists(directory_path):
        # Create the directory
        os.makedirs(directory_path)
    print(f"Directory '{directory_path}' created.")
    ...
    for i, t in enumerate(chunks):
        text_units_section_list.append([i, t["content"]])
    text_units_context = list_of_list_to_csv(text_units_section_list)
    list_of_list_store_json(query, text_units_section_list,global_config["store_naive_text_chunks"])
